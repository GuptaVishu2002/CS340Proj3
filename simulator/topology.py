...
class Topology:
    ...
    def draw_topology(self):
        ...
        filename = 'Topo_' + time.strftime("%H_%M_%S", time.localtime()) + '_Count_' + str(
            self.print_count) + '_Time_' + str(Get_Time()) + '.png'
        self.print_count += 1
    ...
    def get_correct_path(self, source, destination):
        try:
            ...
            shortest_length = nx.algorithms.shortest_path_length(self.__g, source=source, target=destination,
                                                                 weight='latency')
        except:
